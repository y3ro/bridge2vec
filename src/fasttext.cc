...
void FastText::getWordVectorNoSkip(Vector& vec, const std::string& word) const {
  const std::vector<int32_t>& ngrams = dict_->getSubwordsNoSkip(word);
  vec.zero();
  for (int i = 0; i < ngrams.size(); i ++) {
    addInputVector(vec, ngrams[i]);
  }
...
void FastText::skipgram(Model& model, real lr,
  ...
  for (int32_t w = 0; w < line.size(); w++) {
    ...
    for (int y = 0; y < sw.size(); y++) {
        	const std::vector<int32_t>& sngrams = dict_->getSubwords(sw[y]); 
        	sngramss.push_back(sngrams);
    }
...
void FastText::train(const Args args) {
  ...
  if (args_->phon_dict.size() != 0) {
    std::ifstream pfs(args_->phon_dict);
    if (!pfs.is_open()) {
      throw std::invalid_argument(
          args_->phon_dict + " cannot be opened for training!");
    }
    dict_->readExtraDict(pfs);
    pfs.close();
  }
