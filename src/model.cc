...
real Model::negativeSamplingSynth(int32_t target, real lr) {
  real loss = 0.0;
  grad_.zero();
  for (int32_t n = 0; n <= args_->neg; n++) {
    if (n == 0) {
    loss += binaryLogistic(target, true, lr);
      ...
      loss += binaryLogistic(getNegative(target), false, lr * 0.01);
    }
  }
  return loss;
}

real Model::negativeSampling(int32_t w, std::shared_ptr<Dictionary> dict, int32_t target, real lr) {
  real loss = 0.0;
  grad_.zero();
  int negs2 = args_->neg;
  
  loss += binaryLogistic(target, true, lr); 
  for (int32_t n = 0; n < negs2; n++) {
    loss += binaryLogistic(getNegative(target), false, lr);
  }
...
real Model::negativeSampling2(int32_t w, std::shared_ptr<Dictionary> dict, int32_t target, real lr) {
  real loss = 0.0;
  grad_.zero();
  int negs2 = args_->neg;
  
  loss += binaryLogistic(w, true, lr); //ctxt positive example
  for (int32_t n = 0; n < negs2; n++) {
    loss += binaryLogistic(getNegative(w), false, lr);
  }
...
void Model::updateWnoise(const std::vector<int32_t>& input, const std::vector<std::vector<int32_t>>& noises, int32_t target, real lr) {
  assert(target >= 0);
  assert(target < osz_);
  if (input.size() == 0) return;
  computeHidden(input, hidden_);
  if (args_->loss == loss_name::ns) {
    loss_ += negativeSampling(target, lr);
    for (int y = 0; y < noises.size(); y++) {
      computeHidden(noises[y], hidden_);
      loss_ += negativeSampling(target, lr) * 0.1; 
    }
  } else if (args_->loss == loss_name::hs) {
    loss_ += hierarchicalSoftmax(target, lr);
  } else {
  ...
  for (int y = 0; y < noises.size(); y++)
  for (auto it = noises[y].cbegin(); it != noises[y].cend(); ++it) {
      wi_->addRow(grad_, *it, 1.0);
  }
}

void Model::updateSynth(const std::vector<int32_t>& input, int32_t target, real lr) {
  assert(target >= 0);
  assert(target < osz_);
  if (input.size() == 0) return;
  computeHidden(input, hidden_);
  if (args_->loss == loss_name::ns) {
    loss_ += negativeSamplingSynth(target, lr);
  } else if (args_->loss == loss_name::hs) {
    loss_ += hierarchicalSoftmax(target, lr);
  } else {
    loss_ += softmax(target, lr);
...
void Model::update(int32_t w, std::shared_ptr<Dictionary> dict, const std::vector<int32_t>& input, int32_t target, real lr) {
  assert(target >= 0);
  assert(target < osz_);
  if (input.size() == 0) return;
  computeHidden(input, hidden_);
  if (args_->loss == loss_name::ns) {
    loss_ += negativeSampling(w, dict, target, lr);
  } else if (args_->loss == loss_name::hs) {
    loss_ += hierarchicalSoftmax(target, lr);
  } else {
    loss_ += softmax(target, lr);
...
void Model::update2(int32_t w, std::shared_ptr<Dictionary> dict, const std::vector<int32_t>& input, int32_t target, real lr) {
  assert(target >= 0);
  assert(target < osz_);
  if (input.size() == 0) return;
  computeHidden(input, hidden_);
  if (args_->loss == loss_name::ns) {
    loss_ += negativeSampling2(w, dict, target, lr);
  } else if (args_->loss == loss_name::hs) {
    loss_ += hierarchicalSoftmax(target, lr);
  } else {
    loss_ += softmax(target, lr);
